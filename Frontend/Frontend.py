...
def render_product(product: Product):
    return rx.hstack(
        ...
        rx.text(f"({product['code']}) {product['label']}",
                width="20vw", font_weight="bold"),
        rx.vstack(
            rx.text("Stock:", product["quantity"]),
            rx.text("Categoría:", product["category"]),
            spacing="1",  # Ajustado a un valor permitido
            width="10vw",
        ),
        rx.vstack(
            rx.text("Vendedor:", product["seller"]),
            rx.text("Remitente:", product["sender"]),
            spacing="1",  # Ajustado a un valor permitido
            width="10vw",
        ),
        rx.spacer(),
...
def query_form():
    return rx.vstack(
        rx.hstack(
            rx.text("Consulta:", font_weight="bold"),
            rx.select(
                ["GET", "POST", "PUT", "DELETE"],
        ...
        rx.text("Cuerpo de la consulta:",
                font_weight="bold", margin_top="1rem"),
        rx.text_area(
        ...
        rx.divider(orientation="horizontal", border="solid 1px #ccc",
                   width="100%", margin_y="1rem"),
        rx.hstack(
            rx.text("Estado: ", QueryState.response_code, font_weight="bold"), rx.spacer(), width="100%"
        ),
        rx.container(
            rx.markdown(
...
def index() -> rx.Component:
    return rx.hstack(
        ...
        data_display(),
        rx.spacer(),
        rx.divider(orientation="vertical", border="solid 1px #ccc"),
        query_form(),
        rx.spacer(),
        height="100vh",
        width="100vw",
        spacing="2",  # Ajustado a un valor permitido
        padding="2rem",
    )
